...
class RegisterForm(FlaskForm):
    add_choices = [('강남구', '강남구'), ('강동구', '강동구'), ('강북구', '강북구'), ('강서구', '강서구'), ('관악구', '관악구'), 
                   ...
                   ('용산구', '용산구'), ('은평구', '은평구'), ('종로구', '종로구'), ('중랑구', '중랑구'), ('중구', '중구')]
...
def store():
    ...
    if request.method == 'POST':
        ...
        if user:
            ...
            sql = "select culture_view, culture_parti, sport_view, sport_parti , sightsee, entertain, rest , social_act from question_score"
            result = db_engine.execute(sql)
            question_scores = pd.DataFrame(result.fetchall(),
                                            columns=(
                                            '문화예술관람활동', '문화예술참여활동', '스포츠관람활동', '스포츠참여활동', 
                                            '관광활동', '취미오락활동', '휴식활동', '사회활동'))
            ...
            db.session.commit()
            new_form = LoginForm()
    return render_template('index.html', form=new_form, les_data=program_rand, job_data=job_rand, act_data=act_rand)

# shows 'comming soon' page before given date
# @app.route('/')
# def home():
...
def search_activities():
    ...
    local = request.args.get("local", "0")
    category = request.args.get("category", "0")
    data = activity_search(email, name, local, category)
    random_listing = define_listing()

    return render_template("search-activities.html", data=data, len=len(data), random_listing=random_listing, email=email)
...
def search_leisures():
    ...
    if category == "야외":
        data = activity_search(email, name, "0", "0")
        random_listing = define_listing()
        return render_template("search-activities.html", data=data, len=len(data), random_listing=random_listing, email=email)
    else:
        _, data = welfare_search(email, name, "0", "0")
        random_listing = define_listing()

        return render_template("search-program.html", data=data, len=len(data), random_listing=random_listing,
                               email=email)
